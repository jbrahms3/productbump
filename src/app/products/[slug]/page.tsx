...
export default async function ProductPage({ params }: Props) {
  ...
  const queued = !product.featured && !product.bumped;

  const transactions = queued
    ? []
  ...
  const queuePosition = queued ? await getQueuePosition(product.createdAt) : null;
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex items-center justify-between">
        ...
        {product.featured && <BumpButton productId={product.id} makerUserId={product.makerUserId} />}
      </div>
      ...
      <div className="card overflow-hidden">
        {queued ? (
          <div className="p-6">
            <p className="text-xs font-medium uppercase tracking-wide text-gray-400">Queue position</p>
            <p className="mt-1 text-3xl font-bold text-gray-900">#{queuePosition}</p>
            <p className="mt-3 text-sm text-gray-500">
              This product isn&apos;t on the homepage yet, so revenue isn&apos;t tracked. Once a slot opens up
              and it&apos;s featured, real Stripe revenue starts counting toward its {formatCurrency(product.revenueThreshold)} goal.
            </p>
          </div>
        ) : (
          <>
